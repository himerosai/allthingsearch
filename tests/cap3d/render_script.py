...
# just load the path here
uid_paths = [args.object_path]
# random.shuffle(uids)
...
def project_points_to_camera_space(obj, camera):
    ...
    for vertex in bbox_world:
        ...
        XYZ_camera = np.dot(RT,
                            np.append(vertex, 1))  # Append 1 to make it a 4-element vector for multiplication with RT
...
for uid_path in uid_paths:
    ...
    for camera_opt in range(-1, 8):
        ...
        if camera_opt == -1:
            ...
            img_path = os.path.join(args.parent_dir, 'Cap3D_imgs', 'Cap3D_imgs_view_bg',
                                    '%s_bg.png' % (uid_path.split('/')[-1].split('.')[0]))
                ...
                for obj in mesh_objects:
                    ...
                    if np.max(np.array(bbox_image) > 512) or np.min(np.array(bbox_image) < 0):
                        ...
                        camera.location = Vector(
                            (distance * ratio, - distance * ratio, distance * elevation_factor * ratio))
                if len(flag_list) == 0:
                    break
        elif camera_opt == 1:
            camera.location = Vector((- distance * ratio, distance * ratio, distance * elevation_factor * ratio))
            ...
            elevation_factor = 0.7
            camera.location = Vector(
                (- distance * ratio * 0.5, -distance * ratio, -distance * elevation_factor * ratio))
        ...
        if camera_opt == -1:
            file_path = os.path.join(args.parent_dir, 'Cap3D_imgs', 'Cap3D_imgs_view_bg',
                                     '%s_bg.png' % (uid_path.split('/')[-1].split('.')[0]))
            bpy.context.scene.render.filepath = file_path
            if os.path.exists(file_path):
                continue
        else:
            file_path = os.path.join(args.parent_dir, 'Cap3D_imgs', 'Cap3D_imgs_view%d' % camera_opt,
                                     '%s_%d.png' % (uid_path.split('/')[-1].split('.')[0], camera_opt))
            bpy.context.scene.render.filepath = file_path
            if os.path.exists(file_path):
                continue
        ...
        if camera_opt >= 0:
            ...
            RT_path = os.path.join(args.parent_dir, 'Cap3D_imgs', 'Cap3D_imgs_view%d_CamMatrix' % camera_opt,
                                   '%s_%d.npy' % (uid_path.split('/')[-1].split('.')[0], camera_opt))
